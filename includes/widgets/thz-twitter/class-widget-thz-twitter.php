...
class Widget_Thz_Twitter extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'description' => esc_html__( 'Twitter widget', 'creatus-extended' ) );
		parent::__construct( false, esc_html__( 'Creatus - Twitter', 'creatus-extended' ), $widget_ops );
	...
	function form( $instance ) {
		...
		?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php echo esc_html__('Title','creatus-extended') ?>:</label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
        </p>
		<p>
        <label for="<?php echo esc_attr( $this->get_field_id( 'apikeys' ) ); ?>"><?php esc_html_e( 'Api keys:', 'creatus-extended' ); ?></label>
        <select name="<?php echo esc_attr( $this->get_field_name( 'apikeys' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'apikeys' ) ); ?>" class="widefat thz-select-switch">
            <option data-enable=".thz-go-twc-<?php echo $this->id?>" data-disable=".thz-twc-<?php echo $this->id?>" value="theme" <?php selected( $instance['apikeys'], 'theme' ); ?>><?php esc_html_e( 'Use theme api keys', 'creatus-extended' ); ?></option>
            <option data-enable=".thz-twc-<?php echo $this->id?>" data-disable=".thz-go-twc-<?php echo $this->id?>" value="custom" <?php selected( $instance['apikeys'], 'custom' ); ?>><?php esc_html_e( 'Insert custom api keys', 'creatus-extended' ); ?></option>
        </select>
		</p>
		<p class="thz-go-twc-<?php echo $this->id?>">
        	<a href="<?php echo self_admin_url( 'themes.php?page=fw-settings#fw-options-tab-advanced') ?>">
        		<?php echo esc_html__('Please go to theme settings and add you Twitter App api keys','creatus-extended'); ?>
            </a>
        </p>
        <div class="thz-twc-<?php echo $this->id?>">
			<p><?php echo esc_html__('To obtain Twitter API keys please visit','creatus-extended') ?> <a href="https://dev.twitter.com/apps" target="_blank">https://dev.twitter.com/apps</a> <?php echo esc_html__('and create new application','creatus-extended') ?>.
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('consumer_key')); ?>"><?php echo esc_html__('Consumer Key','creatus-extended') ?>:</label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('consumer_secret')); ?>"><?php echo esc_html__('Consumer Secret','creatus-extended') ?>:</label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>"><?php echo esc_html__('Access Token','creatus-extended') ?>:</label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('access_token_secret')); ?>"><?php echo esc_html__('Access Token Secret','creatus-extended') ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('twitter_id')); ?>"><?php echo esc_html__('Twitter User','creatus-extended') ?>:</label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php echo esc_html__('Number of Tweets','creatus-extended') ?>:</label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('tweet_limit')); ?>"><?php echo esc_html__('Tweet characters limit','creatus-extended') ?>:</label>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id( 'tweet_link',1 ); ?>"><?php esc_html_e('Display tweet as link','creatus-extended') ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id( 'haslist',1 ); ?>"><?php esc_html_e('Use .thz-has-list class','creatus-extended') ?></label>
        </p>
